...
class MasterAgent:
    ...
    def __init__(self):
        ...
        self.market_agent = MarketAgentHybrid(
            use_rag=True,
            use_web_search=True,
            initialize_corpus=False  # Corpus already exists
        )
        logger.info("Master Agent initialized with Clinical and Market agents")

    def _classify_query(self, query: str) -> List[str]:
        """
        Classify query to determine which agents to activate.

        Returns: List of agent IDs: ['market'], ['clinical'], or ['market', 'clinical']

        Classification Logic:
        - FTO/multi-dimensional queries: Always run both agents
        ...
        - Default: Both agents (conservative approach for comprehensive coverage)
        """
        query_lower = query.lower()

        # CRITICAL: FTO and multi-dimensional queries ALWAYS require both agents
        # These queries need comprehensive IP/patent landscape AND clinical trial data
        ...
        if is_multi_dimensional:
            logger.info("🎯 Query classified as: MULTI-DIMENSIONAL (FTO/Comprehensive) → BOTH AGENTS")
    ...
    def _fuse_results(self, query: str, results: Dict[str, Any], execution_status: List[Dict[str, Any]] = None) -> Dict[str, Any]:
        """
